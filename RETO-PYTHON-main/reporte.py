...
def generar_reporte():
    try:
        ...
        if not datos:
            ...
            return
        materias_unicas = sorted(list({m["materia"].capitalize() for m in datos})) # Crea un conjunto de materias únicas y luego convertirlo en una lista ordenada
        total_materias_unicas = len(materias_unicas)
        total_sesiones = len(datos)
        total_horas = sum(int(m["hora_fin"]) - int(m["hora_inicio"]) for m in datos) # Calcular la duración de cada materia y sumarlas para obtener el total de horas
        horas_por_dia = {}
        for m in datos:
            dia = m["dia"].capitalize()
            duracion = int(m["hora_fin"]) - int(m["hora_inicio"])
            horas_por_dia[dia] = horas_por_dia.get(dia, 0) + duracion # Valida que el día ya exista en el diccionario y acumula las horas correspondientes, si no existe, lo inicializa en 0 y luego suma la duración de la materia
        dia_mas_cargado = max(horas_por_dia, key=horas_por_dia.get) if horas_por_dia else "N/A" #Compara los valores del diccionario y devuelve la clave con el valor máximo, si el diccionario está vacío, devuelve "N/A"
        reporte = {
            ...
            "Detalle_Horarios": [
                ...
                for m in datos # Genera una lista de diccionarios con los detalles de cada materia, incluyendo la hora de inicio y fin en formato de 24 horas
            ]
        }
